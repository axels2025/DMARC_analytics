...
-- RLS policies for trusted_ips
CREATE POLICY IF NOT EXISTS "Users can view their own trusted IPs"
ON public.trusted_ips
FOR SELECT
TO authenticated
USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can insert their own trusted IPs"
ON public.trusted_ips
FOR INSERT
TO authenticated
WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update their own trusted IPs"
ON public.trusted_ips
FOR UPDATE
TO authenticated
USING (auth.uid() = user_id)
...
CREATE POLICY IF NOT EXISTS "Users can delete their own trusted IPs"
ON public.trusted_ips
FOR DELETE
TO authenticated
USING (auth.uid() = user_id);
...
-- RLS: authenticated users can read global rows (user_id IS NULL) and their own rows
CREATE POLICY IF NOT EXISTS "Users can view their own and global classifications"
ON public.ip_classifications
FOR SELECT
TO authenticated
USING (user_id IS NULL OR user_id = auth.uid());

-- Only owners can write their rows
CREATE POLICY IF NOT EXISTS "Users can insert their own classifications"
ON public.ip_classifications
FOR INSERT
TO authenticated
WITH CHECK (user_id = auth.uid());

CREATE POLICY IF NOT EXISTS "Users can update their own classifications"
ON public.ip_classifications
FOR UPDATE
TO authenticated
...
CREATE POLICY IF NOT EXISTS "Users can delete their own classifications"
ON public.ip_classifications
FOR DELETE
TO authenticated
USING (user_id = auth.uid());
